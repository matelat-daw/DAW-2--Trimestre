...
$hash = $urlArray[5]; // La URL es: localhost/Programación/PHP/Ejercicio-2/activate.php/#hash/ID - Por eso se toman los valores 5 y 6 del Array el hash es el Índice 5.
$id = $urlArray[6]; // La ID es el Índice 6.

$stmt = $conn->prepare("SELECT * FROM user WHERE id=$id;"); // Compruebo si la ID que Llego por la URL Está Registrada en la Base de Datos.
$stmt->execute();
if ($stmt->rowCount() > 0) // Si Está.
{
    ...
    {		
        $stmt = $conn->prepare("UPDATE user SET hash=NULL, active=true WHERE id=$id;"); // Sentencia para Actualizar los Datos.
        $stmt->execute();
