<?php
	...
	switch(true) // Hago un switch a la solicitud que llega.
	{
		case isset($_GET["create"]): // Si es create.
			...
			{
				$sql = "INSERT INTO contacto (name, surname, surname2, phone, email, pass, bday, gender, path) VALUES('$name', '$surname', '$surname2', '$phone', '$email', '$hash', '$bday', '$gender', '$path')";
		...
		case isset($_GET["update"]): // Si llega la solicitud update.
			...
			if (!$already) // Si $already es false.
			{
				if ($email == $old_email) // Verifico si el email es el mismo que ya estaba.
				{
					...
					{
						$sql = "UPDATE contacto SET name='$name', surname='$surname', surname2='$surname2', pass='$hash', bday='$bday', gender='$gender', path='$path' WHERE id=$id;";
					...
					{
						$sql = "UPDATE contacto SET name='$name', surname='$surname', surname2='$surname2', phone='$phone', pass='$hash', bday='$bday', gender='$gender', path='$path' WHERE id=$id;";
					}
				}
				else if ($phone == $old_phone) // Si el email es distinto pero el phone es el mismo.
				{
					$sql = "UPDATE contacto SET name='$name', surname='$surname', surname2='$surname2', email='$email', pass='$hash', bday='$bday', gender='$gender', path='$path' WHERE id=$id;";
				}
				else // Si es distinto el email y el teléfono.
				{
					$sql = "UPDATE contacto SET name='$name', surname='$surname', surname2='$surname2', phone='$phone', email='$email', pass='$hash', bday='$bday', gender='$gender', path='$path' WHERE id=$id;";
				}
				$stmt = $conn->prepare($sql);
				$stmt->execute();
		...
		case isset($_GET["delete"]): // Si la solicitud es delete.
			...
			if($stmt->rowCount() > 0) // Verifico si se ha modificado la cantidad de filas.
			{
                ...
                $sql = "SET @count = 0; UPDATE contacto SET id = @count:= @count + 1; ALTER TABLE contacto AUTO_INCREMENT = 1;"; // Arreglo los índices de los Usuarios.
                $stmt = $conn->prepare($sql);
                $stmt->execute();
		...
		case isset($_GET["login"]): // Si llega una solicitud de login.
			...
			{
				response(200, "Bienvenido Usuario: ", array($row->id, $row->name, $row->surname, $row->surname2, $row->phone, $email, $pass, $row->bday, $row->gender, $row->path));
