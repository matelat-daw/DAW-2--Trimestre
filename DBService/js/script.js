...
{
    let alertaImg = document.getElementById("alertaImg"); // La ID del botón del dialogo.
    let img = document.getElementById("show_pic"); // Asigno a la variable title el h4 con id title.
...
function totNumPages()
{
    ...
    function change(page, qtty, length)
    {
        ...
        var html = "<table><tr><th>ID</th><th>Nombre</th><th>Apellido 1</th><th>Apellido 2</th><th>Teléfono</th><th>E-mail</th><th>Cumpleaños</th><th>Genero</th><th>Imágen de Perfil</th></tr><tr>";
        for (i = (page - 1) * qtty; i < page * qtty; i++)
        {
            if (i < length)
            {
                ...
                html += "<td>" + id[i] + "</td><td>" + username[i] + "</td><td>" + surname[i] + "</td><td>" + surname2[i] + "</td><td>" + phone[i] + "</td><td>" + email[i] + "</td><td>" + bday[i] + "</td><td>" + gen + "</td><td><a href='javascript:showImg(\"" + img[i] + "\")'><img src='" + img[i] + "' width='100' height='100' alt='Imágen de Perfil'></a></td></tr><tr>";
            }
        }
        html += "</tr></table>";
