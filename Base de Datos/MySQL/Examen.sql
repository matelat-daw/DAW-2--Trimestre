...
ADD CONSTRAINT FK_Asignacion_Tarea FOREIGN KEY (cod_tarea) REFERENCES tareas (cod_tarea) ON DELETE NO ACTION ON UPDATE CASCADE; -- Agregada Posteriormente.
...
ADD CONSTRAINT FK_Registro_Funcionario FOREIGN KEY (cod_func) REFERENCES personal(cod_func) ON DELETE NO ACTION ON UPDATE CASCADE;

ALTER TABLE clientes
ADD CONSTRAINT FK_Cliente_Personal FOREIGN KEY (cod_func) REFERENCES personal(cod_func) ON DELETE NO ACTION ON UPDATE CASCADE;
...
ALTER TABLE proyectos
ADD COLUMN Descripcion VARCHAR(50) UNIQUE NOT NULL;
...
UPDATE personal SET fecha_ingreso=NOW() WHERE personal.cod_func=223;
-- o
UPDATE personal SET fecha_ingreso=NOW() WHERE personal.nombre="TERESA";
