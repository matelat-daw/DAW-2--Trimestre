...
if (isset($_POST["email"]))
{
    ...
    $ok = false;
    $sql = "SELECT client_email FROM client;"; // Preparo una consulta de todos los E-mail de espectadores de la base de datos.
    $stmt = $conn->prepare($sql);
    $stmt->execute();
    while($row = $stmt->fetch(PDO::FETCH_OBJ)) // Asigno a la variable $row el contenido de la consulta.
    {
        if ($row->client_email == $email) // Si el E-mail está en la base de datos.
        {
            $ok = true; // $ok es true.
    ...
    {
        $sql = "UPDATE client SET client_pass='$pass' WHERE client_email='$email';"; // Hago un update de la contraseña de ese E-mail.
        $stmt = $conn->prepare($sql);
        $stmt->execute();
