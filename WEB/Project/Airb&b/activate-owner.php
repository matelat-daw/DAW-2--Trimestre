...
if (isset($_GET["hash"]))
{
    ...
    $stmt = $conn->prepare("SELECT * FROM owner WHERE owner_dni='$id';");
    $stmt->execute();
    if ($stmt->rowCount() > 0)
    {
        $row = $stmt->fetch(PDO::FETCH_OBJ);
        if ($row->owner_hash == $hash)
        {		
            $stmt = $conn->prepare("UPDATE owner SET owner_hash=NULL, owner_active=1 WHERE owner_dni='$id';");
            $stmt->execute();
