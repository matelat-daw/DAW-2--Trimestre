...
if (isset($_POST["username"])) // Si se ha Enviado por POST el username.
{
    ...
    $user = $_POST["username"]; // Asigno a distintas Variables los Datos que Llegan por POST.
    ...
    $sql = "SELECT dni, phone, email FROM user WHERE phone='$phone' OR email='$email' OR dni='$dni'"; // Esta Sentencia SQL Verifica que no Estén ya Registrados Teléfono, E-mail o D.N.I.
...
if ($ok) // Si $ok es true.
{
    ...
    $sql = "INSERT INTO user VALUES(:id, :dni, :name, :surname, :surname2, :phone, :email, :pass, :bday, :path, :hash, :active)"; // Inserta en la Base de Datos.
    $stmt = $conn->prepare($sql);
